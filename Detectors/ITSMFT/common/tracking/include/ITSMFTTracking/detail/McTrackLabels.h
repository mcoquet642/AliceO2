...
inline bool isMftOnlyLayout(gsl::span<const SurfaceDescriptor> layers) noexcept
{
  if (layers.empty()) {
    return false;
  }
  constexpr auto mftId = static_cast<uint8_t>(o2::detectors::DetID::MFT);
  return std::all_of(layers.begin(), layers.end(), [mftId](const SurfaceDescriptor& surface) {
    return surface.detectorId == mftId;
  });
}
